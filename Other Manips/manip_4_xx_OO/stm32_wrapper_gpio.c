...
{
	int pinID = (pin & 0x0F);
	int pinValue =  (AIRget_gpioport(pin)->IDR) & (1<<pinID);
	return (pinValue) ;
}
...
static void _gpio_init_in(PinName pin, PinInMode mode)
{
	...
	GPIO_TypeDef* GPIOx = AIRget_gpioport(pin);
...
static GPIO_TypeDef* AIRget_gpioport (PinName pin)
{
	int gpioID = (pin & 0xF0) >> 4; 
...
void AIRgpio_init_out_def(PinName pin)
{
    AIRgpio_init_out (pin, OutDefault);
}


/* With Mode , Default Speed = 2 Mhz  */
void AIRgpio_init_out (PinName pin, PinOutMode mode)
{
    AIRgpio_init_out_ex (pin, mode, SpeedDefault);
}

/*  With Mode and Speed */
void AIRgpio_init_out_ex (PinName pin, PinOutMode mode, PinSpeed speed)
{
    AIR_gpio_init_out(pin, mode, speed);
}
...
int AIRgpio_read_out(PinName pin)
{
//Code to complete
	int pinID = (pin & 0x0F);
	
	return ( (AIRget_gpioport(pin)->ODR) & (1<<pinID)) ;
}


/*==== Function To write Pin Value To ODR === */
/*--------------------------------------------*/
...
/*--------------------------------------------*/
	...
	void AIRgpio_write(PinName pin, int value)
{
//Code to complete
	int pinID = (pin & 0x0F);
	if (value == 0){
		AIRget_gpioport(pin)->BRR= 1<<pinID ; 
	}
	else {
			AIRget_gpioport(pin)->BSRR= 1<<pinID ; 
	}
...
static int AIRget_gpio_out_mode (PinOutMode mode)
{	
		switch (mode){
...
static void AIR_gpio_init_out(PinName pin, PinOutMode mode, PinSpeed speed)
{
// Code to complete  	
	int gpioID = (pin & 0xF0) >> 4; 
	RCC_APB2PeriphClockCmd( 1<<(gpioID+2) , ENABLE);	
	...
	GPIO_InitStruct.GPIO_Mode = (GPIOMode_TypeDef)AIRget_gpio_out_mode(mode);
	
	GPIO_InitStruct.GPIO_Mode = (GPIOMode_TypeDef)AIRget_gpio_out_speed(speed);
