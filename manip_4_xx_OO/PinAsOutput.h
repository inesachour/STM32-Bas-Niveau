...
public:
     ...
     */
		xxDigitalOut(PinName pin) :thepin(pin) { 
			xxgpio_init_out_def  (pin); 
		}
...
/***** Create a PinAsOutput connected to the specified pin
     ...
     */
    xxDigitalOut(PinName pin, PinOutMode mode):thepin(pin) {
        xxgpio_init_out (pin, mode);
    }
...
/***** Create a PinAsOutput connected to the specified pin
     ...
     *  @param mode the initial mode of the pin
     */
    xxDigitalOut(PinName pin, PinOutMode mode, PinSpeed speed):thepin(pin) {
        xxgpio_init_out_ex (pin, mode, speed);
    }
...
/***** Write to the output pin, represented as 0 or 1
     ...
     *    0 for logical 0, 1 for logical 1
     */		
		    void xxwrite(int value) {
					xxgpio_write(thepin,value);
				}
