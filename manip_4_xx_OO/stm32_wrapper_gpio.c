...
{
	int pinID = (pin & 0x0F);
	int pinValue =  (xxget_gpioport(pin)->IDR) & (1<<pinID);
	return (pinValue) ;
}
...
static void _gpio_init_in(PinName pin, PinInMode mode)
{
	...
	GPIO_TypeDef* GPIOx = xxget_gpioport(pin);
...
static GPIO_TypeDef* xxget_gpioport (PinName pin)
{
	int gpioID = (pin & 0xF0) >> 4; 
...
{
    xxgpio_init_out (pin, OutDefault);
}
...
{
    xxgpio_init_out_ex (pin, mode, SpeedDefault);
}

/*  With Mode and Speed */
void xxgpio_init_out_ex (PinName pin, PinOutMode mode, PinSpeed speed)
{
    xx_gpio_init_out(pin, mode, speed);
}
